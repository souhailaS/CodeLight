...
Four things CodeLight could not do before, each one reviewed three times over and every fix held in place by a test.
...
* CodeLight now says what it can do in a workspace that has no files on disk, such as one opened through GitHub Repositories. Notes are read and written normally there; only the check of whether git ignores or carries the annotation file is missing, so nothing is claimed about sharing.
* Four hundred and sixty one tests now run on every push.
